...
require_once __DIR__ . '/coverage-private-runtime.php';
...
const ESKOMI_COVERAGE_WAVE2_MANIFEST_SHA256 = 'dc6e31917309b4d91a478cc1d4fa1d866b7ef8ca849440b3e88eb9ea86f3a4cd';
const ESKOMI_COVERAGE_FIXED_HOLD_IDS = ['M0217', 'M0293', 'M0408', 'M0661'];

function escomi_coverage_fixed_hold_ids(string $batch_id): array
{
    return $batch_id === 'coverage-first-2026-08-25'
        ? ESKOMI_COVERAGE_FIXED_HOLD_IDS
        : [];
}
...
/** Private data stays outside the web root and the theme upload stage. */
function escomi_coverage_manifest_path(?string $batch_id = null)
{
    try {
        $spec = escomi_coverage_private_manifest_specs()[$batch_id ?? 'coverage-first-2026-08-25'] ?? null;
        if ($spec === null) {
            return new WP_Error('batch_not_found', 'Coverage batch was not found.', ['status' => 404]);
        }
        return escomi_coverage_private_root() . '/' . $spec['filename'];
    } catch (Throwable $error) {
        return new WP_Error('coverage_private_unavailable', 'Coverage private prerequisites unavailable.', ['status' => 503]);
    }
}

function escomi_coverage_load_manifest(?string $path = null, ?string $batch_id = null)
{
    $batch_id = $batch_id ?? 'coverage-first-2026-08-25';
    $expected_path = escomi_coverage_manifest_path($batch_id);
    if (is_wp_error($expected_path)) { return $expected_path; }
    if ($path !== null && $path !== $expected_path) {
        return new WP_Error('manifest_unavailable', 'Coverage manifest is unavailable.', ['status' => 503]);
    }
    try {
        $manifest = escomi_coverage_private_manifest($batch_id);
    } catch (Throwable $error) {
        return new WP_Error('coverage_private_unavailable', 'Coverage private prerequisites unavailable.', ['status' => 503]);
    }
    $valid = escomi_coverage_validate_manifest_contract($manifest);
    return is_wp_error($valid) ? $valid : $manifest;
}
...
function escomi_coverage_validate_manifest_contract(array $manifest)
{
    ...
    $profiles = [
        'coverage-first-2026-08-25' => [
            'candidate_rows' => 30,
            'execution_entities' => 28,
            'pilot_operations' => 9,
            'operation_pattern' => '/\Acoverage-m[0-9]+-(update|create|relation)\z/',
            'evidence_sources' => false,
        ],
        'coverage-wave2-2026-08-28' => [
            'candidate_rows' => 22,
            'execution_entities' => 22,
            'pilot_operations' => 0,
            'operation_pattern' => '/\Acoverage-wave2-m[0-9]+-(update|create|relation)\z/',
            'evidence_sources' => true,
        ],
    ];
    $profile = $profiles[(string) ($manifest['batch_id'] ?? '')] ?? null;
    if (!is_array($profile)
        || !escomi_coverage_exact_keys($manifest, $top_keys)
        || ($manifest['schema_version'] ?? null) !== 1
        || ($manifest['mode'] ?? null) !== 'DRY_RUN_ONLY'
        || ($manifest['candidate_row_count'] ?? null) !== $profile['candidate_rows']
        || ($manifest['execution_entity_count'] ?? null) !== $profile['execution_entities']
        ...
        || count($manifest['operations']) !== $profile['execution_entities']
        || count($manifest['candidate_rows']) !== $profile['candidate_rows']
        || !is_array($manifest['pilot_operation_ids'] ?? null)
        || count($manifest['pilot_operation_ids']) !== $profile['pilot_operations']
    ) {
    ...
    if ($profile['evidence_sources']) {
        $payload_base_keys[] = 'evidence_sources';
    }
    ...
    foreach ($manifest['operations'] as $operation) {
        ...
        $allowed_statuses = [
            'UPDATE_EXISTING' => $profile['evidence_sources'] ? ['READY_UPDATE', 'NO_CHANGE', 'HOLD'] : ['READY_UPDATE'],
        ...
        if (!isset($allowed_statuses[$action])
            ...
            || !preg_match($profile['operation_pattern'], (string) ($operation['operation_id'] ?? ''))
        ...
        if ($action === 'ADD_AREA_RELATION') {
            ...
            if ($profile['evidence_sources']) {
                $payload_keys[] = 'allowed_identity_collisions';
            }
        }
        ...
        if ($profile['evidence_sources']) {
            if (!is_array($payload['evidence_sources'] ?? null) || !$payload['evidence_sources']) {
                return new WP_Error('manifest_invalid', 'Evidence source list mismatch.', ['status' => 503]);
            }
            foreach ($payload['evidence_sources'] as $evidence_source) {
                if (!is_array($evidence_source)
                    || !escomi_coverage_exact_keys($evidence_source, ['observed_at', 'purpose', 'source'])
                    || ($evidence_source['purpose'] ?? '') !== 'identity_location'
                    || !str_starts_with((string) ($evidence_source['source'] ?? ''), 'https://')
                    || ($evidence_source['observed_at'] ?? '') === ''
                ) {
                    return new WP_Error('manifest_invalid', 'Evidence source contract mismatch.', ['status' => 503]);
                }
            }
            if ($action === 'CREATE_NEW') {
                if (!is_array($payload['allowed_identity_collisions'] ?? null)) {
                    return new WP_Error('manifest_invalid', 'Allowed identity collision contract mismatch.', ['status' => 503]);
                }
                foreach ($payload['allowed_identity_collisions'] as $allowed_collision) {
                    if (!is_array($allowed_collision)
                        || !escomi_coverage_exact_keys($allowed_collision, ['fields', 'resolution', 'wp_id'])
                        || !is_int($allowed_collision['wp_id'] ?? null)
                        || ($allowed_collision['wp_id'] ?? 0) < 1
                        || ($allowed_collision['fields'] ?? null) !== ['official_url']
                        || ($allowed_collision['resolution'] ?? '') !== 'RESOLVED_NEW_LOCATION'
                    ) {
                        return new WP_Error('manifest_invalid', 'Allowed identity collision contract mismatch.', ['status' => 503]);
                    }
                }
...
function escomi_coverage_prepare_evidence_sources(array $evidence_sources)
{
    $records = [];
    $resolved = [];
    foreach ($evidence_sources as $item) {
        if (!is_array($item)
            || !escomi_coverage_exact_keys($item, ['observed_at', 'purpose', 'source'])
            || ($item['purpose'] ?? '') !== 'identity_location'
            || ($item['observed_at'] ?? '') === ''
            || !is_string($item['source'] ?? null)
        ) {
            return new WP_Error('manifest_invalid', 'Evidence source contract mismatch.', [
                'status' => 503,
                'failure_scope' => 'SYSTEMIC_BLOCKING',
            ]);
        }
        ...
        $records[] = [
            'field' => 'official',
            'sourceUrl' => (string) $resolved[$source],
            'sourceType' => 'official-site',
            'observedAt' => substr((string) $item['observed_at'], 0, 10),
            'reviewedAt' => gmdate('Y-m-d'),
            'reviewStatus' => 'reviewed',
            'publishedValueHash' => escomi_coverage_payload_hash($source),
        ];
    }
    $sanitized = escomi_sanitize_shop_fact_provenance($records);
    if (!is_array($sanitized)
        || escomi_coverage_canonical_json($sanitized) !== escomi_coverage_canonical_json($records)
    ) {
        return escomi_coverage_provenance_source_error(
            'provenance_source_rejected',
            'Production provenance sanitizer rejected or transformed an evidence record.'
        );
    }
    return [
        'status' => 'PROVENANCE_READY',
...
function escomi_coverage_check_create_collisions(array $operation)
{
    ...
    if (empty($wanted['official_url']) && is_array($payload['evidence_sources'] ?? null)) {
        $first_evidence = $payload['evidence_sources'][0] ?? null;
        if (is_array($first_evidence)) {
            $wanted['official_url'] = escomi_coverage_normalize_identity(
                'official_url',
    ...
    $allowed_by_post = [];
    foreach (is_array($payload['allowed_identity_collisions'] ?? null)
        ? $payload['allowed_identity_collisions']
        : [] as $allowed
    ) {
        $allowed_by_post[(int) ($allowed['wp_id'] ?? 0)] = array_values(array_unique(array_map(
            'strval',
            is_array($allowed['fields'] ?? null) ? $allowed['fields'] : []
        )));
    }
    $target_terms = array_values(array_unique(array_map(
        'intval',
        is_array($payload['area_terms'] ?? null) ? $payload['area_terms'] : []
    )));
    $collisions = [];
    ...
    foreach (is_array($ids) ? $ids : [] as $post_id) {
        ...
        foreach (['official_url', 'shop_tel', 'shop_address'] as $field) {
            if (!empty($wanted[$field])
                ...
                $post_collisions[] = $field;
            }
        }
        if (!$post_collisions) {
            continue;
        }
        $unapproved = array_values(array_diff(
            array_values(array_unique($post_collisions)),
            $allowed_by_post[(int) $post_id] ?? []
        ));
        if (!$unapproved) {
            $current_terms = escomi_coverage_current_area_terms((int) $post_id);
            if (is_wp_error($current_terms)) {
                return $current_terms;
            }
            if (array_intersect($target_terms, $current_terms)) {
                $unapproved = $post_collisions;
            }
        }
        foreach ($unapproved as $field) {
            $collisions[] = $field;
        }
    }
    if ($collisions) {
...
function escomi_coverage_validate_runtime_operation(array $manifest, array $operation, array $params)
{
    ...
    $action = (string) ($operation['action'] ?? '');
    if (in_array(
            (string) ($operation['master_shop_id'] ?? ''),
            escomi_coverage_fixed_hold_ids((string) ($manifest['batch_id'] ?? '')),
            true
        )
    ...
    if ($action === 'UPDATE_EXISTING') {
        ...
        $resume_post_id = is_array($ledger) && ($ledger['state'] ?? '') === 'retry_ready'
            ...
            : 0;
        if ($resume_post_id > 0) {
            $recovery_identity = escomi_coverage_recovery_operation_id('failed_create_provenance');
            if ($recovery_identity === null) {
                return new WP_Error('reconcile_contract_unavailable', 'Recovery identity is unavailable.', ['status' => 503]);
            }
            if (($operation['operation_id'] ?? '') === $recovery_identity) {
                $m0240_contract = escomi_coverage_reconcile_contracts()[$recovery_identity] ?? null;
    ...
    if (array_key_exists('evidence_sources', $operation['payload'] ?? [])) {
        $evidence_plan = escomi_coverage_prepare_evidence_sources(
            is_array($operation['payload']['evidence_sources'] ?? null)
                ? $operation['payload']['evidence_sources']
                : []
        );
        if (is_wp_error($evidence_plan)) {
            return escomi_coverage_failure_scope($evidence_plan) === 'CANDIDATE_HOLD'
                ? escomi_coverage_candidate_hold_from_error($evidence_plan, $action, $ledger, $field_contracts)
                : $evidence_plan;
        }
        $validation['evidence_source_plan'] = $evidence_plan;
    }
...
/** Preserve the old normalized record map; never substitute guessed recovery state. */
function escomi_coverage_reconcile_contracts(): array
{
    try {
        return escomi_coverage_private_contracts();
    } catch (Throwable $error) {
        // The route checks prerequisites before dispatch. Direct internal lookups also fail closed.
        return [];
    }
}

/** Resolve the existing recovery branch's identity from its private record. */
function escomi_coverage_recovery_operation_id(string $kind): ?string
{
    foreach (escomi_coverage_reconcile_contracts() as $identity => $contract) {
        if ($contract['reconcile_kind'] === $kind) { return $identity; }
    }
    return null;
}
...
function escomi_coverage_validate_m0240_failed_draft(array $operation, array $contract)
{
    if (!hash_equals((string) ($contract['payload_hash'] ?? ''), (string) ($operation['payload_hash'] ?? ''))
        || (string) ($operation['operation_id'] ?? '') !== escomi_coverage_recovery_operation_id('failed_create_provenance')
...
function escomi_coverage_handle_request($request)
{
    ...
    try {
        escomi_coverage_private_contracts();
        foreach (array_keys(escomi_coverage_private_manifest_specs()) as $private_batch) {
            $private_manifest = escomi_coverage_private_manifest($private_batch);
            $valid = escomi_coverage_validate_manifest_contract($private_manifest);
    ...
    $manifest = escomi_coverage_load_manifest(null, $params['batch_id']);
