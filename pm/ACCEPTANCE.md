...
- 地域Hubページから店舗一覧、ランキング、料金比較、初心者向け、関連エリアへ移動できる。
- 地域詳細の店舗カードから店舗詳細へ移動できる。
- 店舗詳細の上部導線から同エリアランキング、同エリア料金比較、口コミ投稿へ移動できる。
- 店舗詳細下部の関連ページ導線がランキング、料金比較、口コミへ接続されている。
- `npm run test:internal-links` を追加し、`npm test` に接続済み。
