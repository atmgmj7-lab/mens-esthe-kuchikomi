import type { SearchConsoleKeywordMetric } from "@/lib/searchConsole";
import { formatNumber, formatPercent } from "@/lib/ga";
import { periodLabel } from "@/lib/period";
import type { PeriodDays } from "@/lib/period";

type Props = {
  items: SearchConsoleKeywordMetric[];
  loading?: boolean;
  period: PeriodDays;
  error?: string;
};

export default function SearchKeywordTable({ items, loading, period, error }: Props) {
  return (
    <div className="rounded-xl bg-zinc-900 border border-zinc-800 p-5 overflow-x-auto">
      <p className="text-sm font-medium text-zinc-300 mb-4">
        Search Console キーワードTOP（{periodLabel(period)}）
      </p>
        ...
        <table className="w-full text-sm min-w-[760px]">
          <thead>
            <tr className="text-left text-xs text-zinc-500 border-b border-zinc-800">
              ...
              <th className="pb-2 pr-3 font-medium">クエリ</th>
              ...
              <th className="pb-2 font-medium text-right">平均掲載順位</th>
            </tr>
          </thead>
          <tbody>
            {items.map((item, i) => (
              <tr key={`${item.query}-${i}`} className="border-b border-zinc-800/70 last:border-0">
                <td className="py-2.5 pr-3 text-zinc-500 tabular-nums">{i + 1}</td>
                <td className="py-2.5 pr-3 text-zinc-200 font-medium">{item.query}</td>
                <td className="py-2.5 pr-3 text-zinc-500 max-w-[220px] truncate">{item.page}</td>
                <td className="py-2.5 pr-3 text-right tabular-nums text-zinc-300">{formatNumber(item.impressions)}</td>
                <td className="py-2.5 pr-3 text-right tabular-nums text-emerald-400">{formatNumber(item.clicks)}</td>
                <td className="py-2.5 pr-3 text-right tabular-nums text-zinc-400">{formatPercent(item.ctr)}</td>
                <td className="py-2.5 text-right tabular-nums text-zinc-400">{item.position > 0 ? item.position.toFixed(2) : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
