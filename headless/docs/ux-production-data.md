...
| Approved user reviews | Existing read-only WordPress `reviews` contract | Reuse only `publish` and `approved` results whose canonical shop relation points to a public shop. The shop reader binds the requested shop ID. The unfiltered global response preserves its deployed shop-and-area shape; an Area-filtered response also returns the validated explicit Primary Area. Both local source wrappers reject caller-assembled objects. |
...
The global latest-review feed is `GET /wp-json/escomi/v1/reviews`, with bounded `page` and `per_page` plus optional `primary_area_slug`. The default and maximum page size are 20, and page is capped at 1000 before a cache entry or WordPress offset is created. WordPress filters `publish` plus `approved` reviews through exactly one stored positive shop ID and exactly one approval value, requires the related shop to be public, orders by published date and review ID descending, and resolves page shops and all assigned `area` terms in bulk. When `primary_area_slug` is present, WordPress first resolves it to the canonical `area` term ID and the same bounded review query requires one canonical positive-integer `shop_primary_area_term_id` value plus that Shop's matching `area` relation. RELATED, UNCLASSIFIED, relation-external, duplicate-Primary, and non-public Shops are excluded. Missing Areas return 404 and malformed slugs return 400 without fallback. Duplicate relation/moderation meta and query failures fail closed. For independent WordPress and Next.js rollout safety, unfiltered WordPress responses keep the previous public payload shape; the Next.js reader accepts that shape and normalizes `primaryArea` to `null`. Filtered responses must include `shop.primaryArea`, and Next.js verifies its slug against the request. The optional slug is part of the cached function arguments, so Area feeds cannot collide. All entries retain the existing `wp` invalidation tag plus `reviews:global`; the reader never calls one Shop endpoint per Shop. Separate Area tags are unnecessary because the existing authenticated `wp` invalidation expires every global and Area-scoped entry immediately.

Both the global and shop review readers use the Next.js `minutes` cache-life profile. In the installed Next.js 16.3.1 defaults this is stale 300 seconds, revalidate 60 seconds, and expire 3600 seconds, but review freshness does not wait for that profile. A public approved-review save, status transition that adds or removes `publish`, deletion of a public approved review, or change to approval, canonical shop relation, or public rating meta queues one request-local shutdown notification. Pending submissions, pending-review rating edits, draft restore, pending deletion, and approval changes with no `approved` value do not invalidate public caches. Public-data invalidation is never discarded by a cross-request throttle; the request-local queue still prevents duplicate sends when one update fires multiple hooks. WordPress sends the authenticated revalidation route the common `wp` tag, and Next invalidates it with `{ expire: 0 }`. Because every global and shop review cache entry carries `wp`, that one invalidation expires both scopes; a second `reviews:global` request is unnecessary. Shop, area, and shop-area relationship addition or removal use the same guaranteed path because their public fields are embedded in the global feed. The configured revalidation secret remains a required deployment condition—missing configuration fails closed and is not treated as a freshness guarantee.
