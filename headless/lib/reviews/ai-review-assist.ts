...
export type AiReviewProvider = Readonly<{ generate(input: AiReviewInput, signal: AbortSignal): Promise<AiReviewOutput | null> }>;
export type AiReviewTelemetry = Readonly<{ model: string; inputTokens: number; outputTokens: number; decision: AiReviewDecision | "UNAVAILABLE"; latencyMs: number; mode: "standard" | "batch" }>;
