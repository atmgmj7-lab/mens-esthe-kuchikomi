...
function verifySourceContract() {
  ...
  const nativeReviewRepository = read("headless/lib/supabase/review-native.ts");
  const nativeSubmitQa = read("headless/scripts/check-review-native-submit-api.mjs");
  ...
  assert.match(reviewSubmitForm, /campaignToken,/);
  assert.match(reviewSubmitRoute, /reviewNativeRepository/);
  assert.match(reviewSubmitRoute, /claimRateLimit/);
  assert.match(reviewSubmitRoute, /campaignToken:\s*validation\.data\.campaignToken\s*\?\?\s*null/);
  assert.doesNotMatch(reviewSubmitRoute, /submitReviewToWordPress|recordPartnerReviewCampaignSubmission/);
  assert.match(nativeReviewRepository, /rest\/v1\/rpc\/\$\{name\}/);
  assert.match(nativeReviewRepository, /record_partner_review_campaign_review/);
  ...
  assert.ok(packageJson.scripts["test:review-native-submit-api"].includes("check-review-native-submit-api.mjs"));
  assert.match(nativeSubmitQa, /same-origin/);
  assert.match(nativeSubmitQa, /Idempotency-Key/);
  assert.match(nativeSubmitQa, /rejected envelopes must not reach Supabase/);
