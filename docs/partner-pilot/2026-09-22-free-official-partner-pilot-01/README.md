...
作成日: 2026-09-22
状態: `USER_APPROVAL_REQUIRED`（準備完了。外部操作は未実施）

この台帳は、最初の Free Official Partner パイロットの**運用順**を決めるための内部資料です。店舗の優劣・公開順位・掲載順位を意味しません。候補を承認するまで、招待、会員作成、Workspace provisioning、連絡、公開、デプロイを行いません。
...
- Production Supabaseに対する読み取り専用照合で、この10件の `wp_shop_id` に既存 `private.partner_workspaces` はありませんでした。従って、現時点で既存Partner Workspaceとの衝突はありません。
...
|---|---|---|---:|---|---|---|---|---|---|---|---|
| P1-1 | Mrs.L’Amant（ミセスラマン） | 新大阪 | 766 | https://mrs-laman-esthe.com/ | 公式サイト / LINE / 電話 / Web予約 | Yes | Yes — 公式サイトとWeb予約導線あり。導入同意後にのみ提供 | Yes — counter QR | `READY_FOR_APPROVAL` | 新大阪、LINE・Web予約の両方、公式/営業時間/予約根拠が9月レビュー済み | 連絡先担当者・サイト設置権限は未確認 |
| P1-2 | BAMBI SPA（バンビスパ） | 堺東 | 725 | https://bambispa.com/ | 公式サイト / LINE / 電話 | Yes | Yes — 公式サイトあり。設置可否は承認後確認 | Yes — counter QR | `READY_FOR_APPROVAL` | 堺東、LINE中心の取得導線、営業時間根拠が9月レビュー済み | 予約導線の詳細とサイト設置権限は未確認 |
...
|---|---|---|---|---|
| P1 | P1-1, P1-2 | ユーザーが2店舗を明示承認し、Production-writeを別途承認 | invitation/login、membership acceptance、Dashboard、QR/URL、最初の実レビュー、AI Assist、moderation、campaign attribution、任意Widget | Critical/Important production issue = 0。wrong-shop attribution、secret exposure、review loss、incorrect publication = 0 |
