...
|---|---|---|---|---|
| P1-01 | release branch | local commitsをmainへfast-forward push | exact SHA、CI、rollback SHA。DB/WordPress/dataは不変 | CI失敗で停止。rollbackは前SHAへの別承認。main SHA readback |
| P1-02 | Vercel Production | P1対応UIをexact SHAでdeploy/promotion | env名存在、staged E2E。DB/secretは不変 | staged failureで停止。前artifactへrollback。alias SHA readback |
...
各行の `USER_APPROVAL_REQUIRED = YES`。migrationはこのBatchで必要と確認されていないため、operationに含めない。
