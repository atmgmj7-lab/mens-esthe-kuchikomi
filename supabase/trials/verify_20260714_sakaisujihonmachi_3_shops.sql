...
begin
  ...
  from app.shops
  where wp_post_id in (695, 709, 1237)
  ...
  where shops.wp_post_id in (695, 709, 1237)
  ...
  where id = '0f17f6a1-3bf4-4d66-8ee6-3f589da4b001'
    and status = 'completed'
    and source_count = 3
    and imported_count = 3
    and failed_count = 0;

  select count(*) into stored_records
  from private.import_records
  where batch_id = '0f17f6a1-3bf4-4d66-8ee6-3f589da4b001'
  ...
  if not stored_shops = 3 then
    raise exception 'Expected 3 draft trial shops, found %', stored_shops;
  end if;
  if not stored_links = 3 then
    raise exception 'Expected 3 non-primary trial area links, found %', stored_links;
  end if;
  if not stored_prices = 2 then
    raise exception 'Expected 2 nonpublic trial prices, found %', stored_prices;
  end if;
  if not stored_images = 2 then
    raise exception 'Expected 2 nonpublic trial images, found %', stored_images;
  end if;
  if not stored_batches = 1 then
    raise exception 'Expected 1 completed trial batch, found %', stored_batches;
  end if;
  if not stored_records = 3 then
    raise exception 'Expected 3 imported trial records, found %', stored_records;
  end if;
end
$$;
